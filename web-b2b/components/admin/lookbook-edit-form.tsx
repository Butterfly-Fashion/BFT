...
import { updateLookbookItemAction } from "@/app/actions";
...
export function LookbookEditForm({ item }: { item: LookbookItem }) {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    const result = await updateLookbookItemAction(fd);
    if (result?.error) {
    ...
    <form onSubmit={handleSubmit} className="mx-auto max-w-2xl grid gap-5">
      ...
      <section className="card p-5">
        <div className="mb-5 border-b border-slate-100 pb-4">
          ...
          <h1 className="mt-1 text-2xl font-black text-slate-900">Edit Image</h1>
        </div>
        ...
        <div className="grid gap-4">
          ...
          <label className="label">
            Title *
            <input className="field" name="title" required defaultValue={item.title} />
          </label>

          <label className="label">
            Description
            <textarea className="field min-h-20" name="description" defaultValue={item.description || ""} />
          </label>

          <div className="grid gap-4 sm:grid-cols-2">
            <label className="label">
              Season / Tag
              <input className="field" name="season" defaultValue={item.season || ""} />
            </label>
            <label className="label">
              Sort order
              <input className="field" name="sort_order" type="number" defaultValue={item.sort_order} min={0} />
            </label>
          </div>

          <label className="label">
            Linked product slug
            <input className="field" name="linked_product_slug" defaultValue={item.linked_product_slug || ""} placeholder="e.g. winter-gloves-black" />
          </label>
