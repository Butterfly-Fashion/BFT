...
export interface PreorderCampaignGroup {
  id: string;
  title: string;
...
export function PreorderList({ campaigns }: { campaigns: PreorderCampaignGroup[] }) {
    ...
    return campaigns
      .map((c) => ({
        ...
        items: c.items.filter((it) =>
          [it.name, it.sku, c.title].filter(Boolean).some((f) => (f as string).toLowerCase().includes(q))
        ),
      }))
    ...
    <>
      ...
      <div className="mb-6 flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-3.5 py-2.5 focus-within:border-slate-400">
        ...
        <input
          ...
          placeholder="Search by product, SKU, or campaign…"
          className="w-full bg-transparent text-sm text-slate-900 outline-none placeholder:text-slate-400"
          aria-label="Search pre-order campaigns"
        />
        {query && (
          <button onClick={() => setQuery("")} className="shrink-0 text-xs font-bold text-slate-400 hover:text-slate-600">
            Clear
          </button>
        )}
      </div>
      ...
      <div className="space-y-8">
        {filtered.map((campaign) => {
          const closesAt = campaign.closesAt ? new Date(campaign.closesAt) : null;
          ...
          return (
            <section key={campaign.id}>
              ...
              {campaign.description && <p className="mb-3 text-sm text-slate-500">{campaign.description}</p>}
                      ...
                      <div className="min-w-0 flex-1">
                        <div className="flex flex-wrap items-start justify-between gap-x-4 gap-y-1">
                          <div className="min-w-0">
                            <p className="truncate font-bold text-slate-900 leading-snug">{item.name}</p>
                        ...
                        <div className="mt-3">
                          ...
                          {!isExpired && (
                            <form action={submitPreorderCommitmentAction} className="flex flex-wrap items-center gap-2">
                              <input type="hidden" name="campaign_id" value={item.campaignId} />
                              <input type="hidden" name="product_id" value={item.productId} />
                              <input type="hidden" name="case_qty" value={item.caseQty} />
                              <div className="flex items-center gap-1.5 rounded-lg border border-slate-200 bg-white px-2.5 py-1.5 focus-within:border-slate-400">
                                <input
                                  ...
                                  required
                                />
                                <span className="text-xs text-slate-400">cases ×{item.caseQty}</span>
                              </div>
                              <button
